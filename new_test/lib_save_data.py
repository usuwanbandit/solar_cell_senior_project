...
def save_set_of_data_sun_constant(set_of_data, version):
    ...
    if len(set_of_data) > 5:
        ...
        simpifly = [int(i) for i in simpifly]
    Pmpp = [];
    Isc = [];
    Voc = [];
    FF = []
    for num, data in enumerate(set_of_data):
        ...
        ax1.set_ylabel("EQE")
        ax1.set_ylim(0, 1.1)
        ax1.set_xlim(350, 3000)
        ax1.legend()
        plt.tight_layout()

        ax1_5.plot(wl * 1e9, data["absorbed"][0], label=f"Total Absorbed mode = {data['mode']} ")
        ax1_5.legend(loc="upper right", frameon=False)
        ax1_5.set_xlabel("Wavelength (nm)")
        ax1_5.set_ylabel("EQE")
        # ax1_5.set_ylim(0, 1.1)
        ...
        Voc.append(data["Voc"])
        FF.append(data["FF"])
        if simpifly != None and num in simpifly:
            for count, i in enumerate(data["allI"]):
        ...
        try:
            ...
            axCar[num].set_title(data["mode"])
            axCar[num].semilogy(data["xsc"][0] * 1e9, data["nsc"][0], 'b', label='e @ short circuit')
            axCar[num].semilogy(data["xsc"][0] * 1e9, data["psc"][0], 'r', label='h @ short circuit')
            axCar[num].semilogy(data["xeq"][0] * 1e9, data["neq"][0], 'b--', label='e @ equilibrium')
            axCar[num].semilogy(data["xeq"][0] * 1e9, data["peq"][0], 'r--', label='h @ equilibrium')

            axCar[num].set_xlabel('Position (nm)')
            axCar[num].set_ylabel('Carrier density (m$^{-3}$)')
            # axCar.legend()
            axCar2[num].set_title(data["mode"])
            axCar2[num].semilogy(data["xsc"][0] * 1e9, data["nsc"][0], 'b', label='e @ short circuit')
            axCar2[num].semilogy(data["xsc"][0] * 1e9, data["psc"][0], 'r', label='h @ short circuit')
            axCar2[num].semilogy(data["xeq"][0] * 1e9, data["neq"][0], 'b--', label='e @ equilibrium')
            axCar2[num].semilogy(data["xeq"][0] * 1e9, data["peq"][0], 'r--', label='h @ equilibrium')
            axCar2[num].set_xlabel('Position (nm)')
            axCar2[num].set_ylabel('Carrier density (m$^{-3}$)')
            plt.tight_layout()
            axCar2.set_xlim(0, 1000e-9)
        except:
