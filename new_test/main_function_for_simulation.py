...
from material_and_layer import *
...
#using function for download file from old result
...
def save_set_of_data_sun_constant(set_of_data, version, focus_area=None):
    ...
    for num, data in enumerate(set_of_data):
        ...
        try:
            if data['con'] != None:
                Pmpp.append(data["iv"]["Pmpp"]/data['con'])
            else:
...
#UI save data 0D use 1 cell sweep con
def simulation0D_sun_sweep(version, cell_ref, plot_note, pdd_options=None, note='' ):
    if pdd_options == None:
        pdd_options = State()
    ...
    for i in plot_note['x_axis']:
        cell = cell_ref
        data_mode = data_solar_cell.copy()
        data_mode['mode'] = f'Light concentration = {i} (sun)'
        data_mode['note'] = note
        data_mode['x_axis'] = plot_note['x_axis']
        data_mode['x_axis_name'] = plot_note["x_axis_name"]
        data_mode['list_structure'].append(
...
#function for sim IV and QE
def savecell(cell, pdd_options, light_source=light_source, pass_qe=False):
    # offset = 0
...
def sim1D_sun_constant(version, sim_mat, plot_note, note, pdd_options=None, old_data=None, simthis=None):  # sc = simulation at 1 sun
    ...
    set_of_data_sun_constant = simulation1D_sun_constant(version, sim_mat, plot_note, note=note,
                                                         pdd_options=pdd_options, old_data=old_data, simthis=simthis)
